...
public class LockContext {
    ...
    public void acquire(TransactionContext transaction, LockType lockType)
        ...
        if (readonly) {
            throw new UnsupportedOperationException("Unsupported: Read Only");
        }
        if (parent != null) {
            if(!LockType.canBeParentLock(lockman.getLockType(transaction, parent.name), lockType)) {
        ...
        if (parent != null) {
            ...
            parent.numChildLocks.put(transaction.getTransNum(), 1 + num);
        }
        return;
    }
    ...
    public void release(TransactionContext transaction)
        ...
        if(numChildLocks.getOrDefault(transaction.getTransNum(), 0) == 0) {
            ...
            throw new InvalidLockException("Multigranularity Violation!");
        }
    ...
    public void promote(TransactionContext transaction, LockType newLockType)
        ...
        if (!LockType.substitutable(newLockType, type) && newLockType != LockType.SIX) {
            throw new InvalidLockException("Promotion Not Accepted!");
        } else if (newLockType == LockType.SIX) {
            if (hasSIXAncestor(transaction)) {
                throw new InvalidLockException("Invalid Lock Exception");
            }else if (type != LockType.S && type != LockType.IX && type != LockType.IS) {
                throw new InvalidLockException("Promotion Not Accepted!");
            }
    ...
    public void escalate(TransactionContext transaction) throws NoLockHeldException {
        ...
        if (readonly) {
            throw new UnsupportedOperationException("Unsupported: Read Only!");
        } else if (explicit == LockType.X || explicit == LockType.S ) {
            return;
        } else if (explicit == LockType.NL) {
            throw new NoLockHeldException("No Lock Exist For This Transactio");
        }
        ...
        if (explicit == LockType.SIX || explicit == LockType.IX) {
            elType = LockType.X;
        }
        lockman.acquireAndRelease(transaction, name, elType, releaseNames);
        numChildLocks.put(transaction.getTransNum(),
                numChildLocks.getOrDefault(transaction.getTransNum(), 0) - releaseCount);
        return;
    }
    ...
    public LockType getEffectiveLockType(TransactionContext transaction) {
        ...
        if (getExplicitLockType(transaction) != LockType.NL) {
            ...
            LockType pType = parent.getEffectiveLockType(transaction);
            if (pType == LockType.SIX) {
                return LockType.S;
            } else if (pType == LockType.IS || pType == LockType.IX) {
                return LockType.NL;
            } else {
                return pType;
            }
    ...
    private List<ResourceName> sisDescendants(TransactionContext transaction) {
        // TODO(proj4_part2): implement
        ArrayList<ResourceName> list = new ArrayList<>();
